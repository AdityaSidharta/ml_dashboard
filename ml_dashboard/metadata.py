...
class Metadata:
    ...
    def load_config(self, config):
        ...
        self.load_prediction(prediction_dict['source'],
        ...
        self.load_groundtruth(groundtruth_dict['source'],
        ...
        for idx, (value) in enumerate(prediction_cards_list):
            if 'aggregation' in value.keys():
                ...
                aggregation = None
            self.insert_cards('prediction', value['row'], value['name'],
                              value['col'], value['type'],
                              value['grouper'], value['granularity'], aggregation)
        for idx, (value) in enumerate(groundtruth_cards_list):
            if 'aggregation' in value.keys():
                aggregation = value['aggregation']
            else:
                aggregation = None
            self.insert_cards('groundtruth', value['row'], value['name'],
                              value['col'], value['type'],
                              value['grouper'], value['granularity'], aggregation)
